...
AudioEngine::AudioEngine(GraphManager *graph) : graph_(graph), blockSize_(512) {
...
void AudioEngine::audioCallback(ma_device *pDevice, void *pOutput,
  ...
  if (!engine || !engine->graph_)
    return;

  float *output = (float *)pOutput;
  engine->graph_->process(frameCount);
  const float *graphOutput = engine->graph_->getNodeOutput("gain1", 0);
  if (graphOutput) {
...
void AudioEngine::stop() { ma_device_uninit(&device_); }
